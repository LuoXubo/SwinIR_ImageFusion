...
class AxialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = torch.split(qkv.reshape(N * W, self.groups, self.group_planes * 2, H),
                              [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=1)
...
class Fusion_network(nn.Module):
    def __init__(self, nC):
        ...
        img_size = 256
        self.fusion_block = FusionBlock_res(nC, img_size)

    def forward(self, en_ir, en_vi):
        f = self.fusion_block(en_ir, en_vi)
        return f
